...
def predict_parking(input_location):
    input_location = [float(i) for i in input_location.split(",")]
    ...
    for i,v in parking_info.iterrows():
        if haversine(input_location[1], input_location[0], v['tw97x'], v['tw97y']) < 1000:
    ...
    parking_data = parking_data.drop(['updatetime', 'date', 'time'], axis=1)
